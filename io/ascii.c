...
int64_t read_ascii(
    ...
    size_t field_size = 8;
    int64_t data_arr_len = 300000;
    int data_idx = 0;

    if (_initial_alloc(field_size, data_arr_len, data) != 0) {
        fprintf(stderr, "Inital alloc failed\n");
        return -1;
    }
    ...
    // Free extra mem at the end
    if (_realloc(field_size, data_idx, data) != 0) {
        fprintf(stderr, "Final realloc failed\n");
        return -1;
    }
...
// Special case of read_asci - if we are doing an autocorrelation the match region
// is just a superset of the data region. So they can both point to the same data,
// and we just put the extra stuff in the match region, but not in the data region
// at the end.
int64_t read_ascii_autocorr(
    ...
    if (_realloc(field_size, data_idx + match_idx, data) != 0) {
        fprintf(stderr, "Final realloc failed\n");
        return -1;
    }
